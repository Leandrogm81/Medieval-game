...
const SECTIONS = [
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 xs:gap-3">
          {[
            ...
            <div key={v.name} className="bg-black/30 p-2 xs:p-3 rounded-lg border border-white/5">
              <span className={`font-bold ${v.color}`}>{v.name}</span>
              <p className="text-[10px] xs:text-xs opacity-70 mt-1">{v.desc}</p>
            </div>
          ))}
        </div>
      </div>
  ...
  },
  {
    ...
    content: (
      <div className="space-y-4">
        <p className="opacity-80 leading-relaxed text-xs xs:text-sm">
        ...
        <div className="space-y-3">
          {[
            ...
            <div key={r.name} className="flex items-start gap-2 xs:gap-3 bg-black/30 p-2 xs:p-3 rounded-lg border border-white/5">
              <r.icon size={16} className={`${r.color} mt-0.5 shrink-0`} />
              <div>
                <span className={`font-bold ${r.color}`}>{r.name}</span>
                <p className="text-[10px] xs:text-xs opacity-70 mt-1">{r.desc}</p>
              </div>
            </div>
          ))}
        </div>
        <div className="bg-black/30 p-2 xs:p-3 rounded-lg border border-white/5">
          <span className="font-bold text-blue-400 flex items-center gap-1.5 xs:gap-2"><Users size={14} /> População</span>
          <p className="text-[10px] xs:text-xs opacity-70 mt-1">Afeta a eficiência produtiva e é consumida ao recrutar tropas. Cresce 7% por turno até o limite da província.</p>
        </div>
      </div>
    )
  },
    ...
    content: (
      <div className="space-y-4">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-[10px] xs:text-xs">
            <thead>
              <tr className="border-b border-white/10 text-[#d4af37]">
                <th className="text-left py-1.5 xs:py-2">Unidade</th>
                <th className="text-center py-1.5 xs:py-2">Custo Leva (10x)</th>
                <th className="text-center py-1.5 xs:py-2">Maint./Turno</th>
                <th className="text-center py-1.5 xs:py-2">ATK / DEF</th>
                <th className="text-center py-1.5 xs:py-2">Requisito</th>
              </tr>
            </thead>
            <tbody className="opacity-80">
              <tr className="border-b border-white/5">
                <td className="py-1.5 xs:py-2 font-bold flex items-center gap-1"><Shield size={10} /> Infantaria</td>
              ...
              <tr className="border-b border-white/5">
                <td className="py-1.5 xs:py-2 font-bold flex items-center gap-1"><Activity size={10} /> Arqueiros</td>
              ...
              <tr>
                <td className="py-1.5 xs:py-2 font-bold flex items-center gap-1"><Zap size={10} /> Cavalaria</td>
        ...
        <div className="bg-red-900/20 p-2 xs:p-3 rounded border border-red-500/20 text-[10px] xs:text-xs italic text-red-200">
  ...
  },
  {
    ...
    content: (
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 gap-1.5 xs:gap-2 text-xs xs:text-sm">
          {[
            ...
            <div key={c.name} className="bg-black/30 p-1.5 xs:p-2 rounded border border-white/5">
              <span className="font-bold text-[#d4af37] text-[10px] xs:text-xs block">{c.name}</span>
              <p className="opacity-70 text-[9px] xs:text-[11px] mt-0.5">{c.desc}</p>
            </div>
          ))}
        </div>
      </div>
    ...
    content: (
      <div className="space-y-2 xs:space-y-3">
        {[
          ...
          <div key={b.name} className="flex items-center justify-between bg-black/30 p-2 xs:p-3 rounded-lg border border-white/5">
            <div className="flex items-center gap-1.5 xs:gap-2">
              <b.icon size={14} className={b.color} />
              <span className={`font-bold text-xs xs:text-sm ${b.color}`}>{b.name}</span>
            </div>
            <div className="text-right text-[10px] xs:text-xs">
              <div className="opacity-60">{b.cost}</div>
              <div className="text-green-400 font-bold">{b.prod}</div>
            </div>
          </div>
        ))}
        <p className="text-[10px] xs:text-xs opacity-60">Todas custam {ACTION_COSTS.build} Pontos de Ação. Produção depende da eficiência (população/máx).</p>
      </div>
    )
  },
  {
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-1.5 xs:gap-2">
          {[
            ...
            <div key={a.name} className="flex justify-between bg-black/30 p-1.5 xs:p-2 rounded-lg border border-white/5 text-xs xs:text-sm">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2 xs:space-y-3 text-xs xs:text-sm">
          {[
            ...
            <div key={i.label} className="bg-black/30 p-2 xs:p-3 rounded-lg border border-white/5">
              <span className="font-bold text-[#d4af37]">{i.label}</span>
              <p className="text-[10px] xs:text-xs opacity-70 mt-1">{i.desc}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-1.5 xs:space-y-2 text-xs xs:text-sm">
          {[
            ...
            <div key={d.action} className="flex items-start gap-1.5 xs:gap-2 bg-black/30 p-1.5 xs:p-2 rounded-lg border border-white/5">
              <span className={`font-bold ${d.color} shrink-0 w-28 xs:w-36`}>{d.action}</span>
              <span className="opacity-70 text-[10px] xs:text-xs">{d.desc}</span>
            </div>
          ))}
        </div>
      </div>
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-2 xs:p-4">
        <motion.div
          ...
          className="bg-[#2c1810] border-2 xs:border-4 border-[#d4af37] rounded-xl xs:rounded-3xl w-full max-w-3xl overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.8)] flex flex-col max-h-[90vh]"
        >
          <div className="p-3 xs:p-5 border-b border-[#d4af37]/30 flex justify-between items-center bg-[#1a0f0a] shrink-0">
            <h2 className="text-lg xs:text-xl font-serif font-bold text-[#d4af37] uppercase tracking-widest flex items-center gap-2 xs:gap-3">
              <Shield size={20} />
              Manual do Soberano
            </h2>
            <div className="flex items-center gap-2 xs:gap-3">
              <span className="text-[10px] xs:text-xs text-[#d4af37]/60 font-bold">{page + 1}/{SECTIONS.length}</span>
          ...
          {/* Page Tabs */}
          <div className="flex gap-1.5 xs:gap-2 px-3 xs:px-5 pt-3 xs:pt-4 pb-1.5 xs:pb-2 overflow-x-auto shrink-0 no-scrollbar">
            {SECTIONS.map((s, i) => {
              const SIcon = s.icon;
              return (
                <button
                  key={i}
                  onClick={() => setPage(i)}
                  className={`flex items-center gap-1.5 xs:gap-2 px-2 xs:px-4 py-1.5 xs:py-2.5 rounded-lg text-[10px] xs:text-xs font-bold uppercase tracking-wider whitespace-nowrap transition-all ${page === i ? 'bg-[#d4af37] text-[#2c1810]' : 'bg-white/5 text-[#f5f2ed]/50 hover:text-[#f5f2ed]/80'
                    }`}
                >
                  <SIcon size={12} />
                  {s.title}
          ...
          {/* Content */}
          <div className="p-3 xs:p-6 overflow-y-auto custom-scrollbar text-[#f5f2ed] flex-1">
            <h3 className="text-base xs:text-lg font-bold text-[#d4af37] border-b border-white/10 pb-1.5 xs:pb-2 mb-3 xs:mb-4 flex items-center gap-1.5 xs:gap-2">
              <Icon size={16} />
              {section.title}
            </h3>
            {section.content}
          ...
          {/* Navigation Footer */}
          <div className="p-2 xs:p-4 border-t border-[#d4af37]/30 bg-[#1a0f0a] flex justify-between items-center shrink-0">
            <button
              onClick={() => setPage(Math.max(0, page - 1))}
              disabled={page === 0}
              className={`flex items-center gap-1.5 xs:gap-2 px-3 xs:px-5 py-2 xs:py-3 rounded-lg xs:rounded-xl text-xs xs:text-sm font-bold transition-all active:scale-95 ${page === 0 ? 'opacity-30 cursor-default' : 'bg-white/10 hover:bg-white/20 text-[#f5f2ed]'
                }`}
            >
            ...
            {page === SECTIONS.length - 1 ? (
              <button
                onClick={onClose}
                className="bg-[#d4af37] hover:bg-[#b8860b] text-[#2c1810] px-4 xs:px-8 py-2 xs:py-3 rounded-lg xs:rounded-xl font-bold transition-colors active:scale-95 shadow-lg shadow-[#d4af37]/20"
              >
                Entendido!
              </button>
            ) : (
              <button
                ...
                className="flex items-center gap-1.5 xs:gap-2 px-3 xs:px-5 py-2 xs:py-3 rounded-lg xs:rounded-xl text-xs xs:text-sm font-bold bg-[#d4af37] hover:bg-[#b8860b] text-[#2c1810] transition-all active:scale-95 shadow-lg shadow-[#d4af37]/20"
              >
