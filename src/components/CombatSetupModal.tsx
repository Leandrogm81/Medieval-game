    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-2 xs:p-4">
        <motion.div
          ...
          className="bg-[#2c1810] border-2 xs:border-4 border-[#d4af37] rounded-xl xs:rounded-3xl w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.8)]"
        >
          {/* Header */}
          <div className="p-3 xs:p-5 border-b border-[#d4af37]/30 flex justify-between items-center bg-[#1a0f0a]">
            <h2 className="text-lg xs:text-xl font-serif font-bold text-[#d4af37] flex items-center gap-2 xs:gap-3 tracking-widest uppercase">
              <Swords size={20} /> Planejamento de Ataque
            </h2>
          ...
          <div className="p-3 xs:p-6 space-y-3 xs:space-y-5 bg-[#2c1810] overflow-y-auto custom-scrollbar flex-1">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 xs:gap-4">
              {/* Atacante */}
              <div className="bg-black/30 p-2 xs:p-4 rounded-lg xs:rounded-xl border border-white/5">
                <h4 className="text-[10px] xs:text-xs font-bold text-blue-400 uppercase tracking-widest mb-2 xs:mb-3 border-b border-white/10 pb-1.5 xs:pb-2">
                  Atacante — {attackerProv.name}
                </h4>
                <div className="space-y-1.5 xs:space-y-2 text-xs xs:text-sm text-[#f5f2ed]">
                  ...
                  <div className="pt-1.5 xs:pt-2 mt-1 border-t border-white/10 flex justify-between font-bold text-blue-400">
              ...
              {/* Defensor */}
              <div className="bg-black/30 p-2 xs:p-4 rounded-lg xs:rounded-xl border border-white/5">
                <h4 className="text-[10px] xs:text-xs font-bold text-red-400 uppercase tracking-widest mb-2 xs:mb-3 border-b border-white/10 pb-1.5 xs:pb-2">
                ...
                <div className="space-y-1.5 xs:space-y-2 text-xs xs:text-sm text-[#f5f2ed]">
                  ...
                  <div className="pt-1.5 xs:pt-2 mt-1 border-t border-white/10 flex justify-between font-bold text-red-400">
            ...
            {/* Modifiers */}
            <div className="bg-black/20 p-2 xs:p-4 rounded-lg xs:rounded-xl border border-[#d4af37]/10 flex flex-col gap-2 xs:gap-3">
              <div className="flex justify-between items-center text-[10px] xs:text-xs">
                ...
                <span className={terrainBonus > 0 ? 'text-red-400 font-bold bg-red-400/10 px-1.5 xs:px-2 py-0.5 rounded' : 'text-slate-500 italic'}>
                  {terrainBonus > 0 ? `+${terrainBonus * 20}% Defesa` : 'Sem bônus'}
                </span>
              </div>
              <div className="flex justify-between items-center text-[10px] xs:text-xs">
                <span className="flex items-center gap-1.5 xs:gap-2 text-[#f5f2ed]/60">
                  <Shield size={12} className="text-[#d4af37]" />
                  Fortificações: <span className="text-[#f5f2ed]">Nível {effectiveDefense}</span>
                </span>
                <span className={effectiveDefense > 0 ? 'text-red-400 font-bold bg-red-400/10 px-1.5 xs:px-2 py-0.5 rounded' : 'text-slate-500 italic'}>
            ...
            {/* Counter Warning */}
            <div className="text-[8px] xs:text-[10px] text-amber-200/50 italic bg-amber-900/10 p-1.5 xs:p-2 rounded border border-amber-500/10">
              * Lembre-se: Cavalaria {'>'} Arqueiros {'>'} Infantaria {'>'} Cavalaria. A composição pode mudar drasticamente o resultado real.
            </div>

            {/* Final Assessment */}
            <div className={`p-2 xs:p-4 rounded-xl xs:rounded-2xl border-2 flex items-center justify-center gap-2 xs:gap-4 shadow-inner ${riskLevel === 'favorable' ? 'bg-green-900/20 border-green-500/40 text-green-300' :
              riskLevel === 'balanced' ? 'bg-amber-900/20 border-amber-500/40 text-amber-300' :
                'bg-red-900/20 border-red-500/40 text-red-300'
              }`}>
              <RiskIcon size={24} />
              <div className="text-center">
                <div className="text-lg xs:text-2xl font-serif font-bold uppercase tracking-tighter">{riskLabel}</div>
                <div className="text-[10px] xs:text-xs opacity-60 font-mono tracking-widest mt-1">
          ...
          {/* Action Buttons */}
          <div className="p-3 xs:p-5 border-t border-[#d4af37]/30 bg-[#1a0f0a] flex gap-2 xs:gap-3">
            <button
              onClick={onClose}
              className="flex-1 py-2 xs:py-3 text-[#f5f2ed]/80 font-bold rounded-lg xs:rounded-xl border border-white/10 hover:bg-white/5 transition-all active:scale-95"
            >
              Recuar
            </button>
            <button
              onClick={onConfirm}
              className="flex-[2] py-2 xs:py-3 bg-[#d4af37] hover:bg-[#b8860b] text-[#2c1810] font-bold rounded-lg xs:rounded-xl shadow-lg shadow-[#d4af37]/20 transition-all active:scale-95 flex items-center justify-center gap-1.5 xs:gap-2"
            >
              <Swords size={16} /> INICIAR ATAQUE
            </button>
          </div>
        </motion.div>
      </div>
