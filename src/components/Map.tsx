...
export const Map: React.FC<MapProps> = ({
  ...
  const getResourceIcon = (resource?: string) => {
    switch (resource) {
      ...
      case 'horses': return <Activity size={10} className="text-amber-600" />;
  ...
  const getProvinceColor = (prov: Province, isVisible: boolean) => {
    ...
    const totalTroops = prov.troops.infantry + prov.troops.archers + prov.troops.cavalry + prov.troops.scouts;
    switch (viewMode) {
      case 'political':
      ...
      case 'economic':
        ...
        return '#4ade80'; // Low
      case 'military':
        if (totalTroops > 100) return '#b91c1c'; // Strong
        if (totalTroops > 50) return '#dc2626'; // Medium
        return '#f87171'; // Weak
      case 'diplomatic':
      ...
      case 'resources':
        ...
        if (prov.strategicResource === 'horses') return '#92400e';
  ...
  const renderArmyIcon = (prov: Province) => {
    const total = prov.troops.infantry + prov.troops.archers + prov.troops.cavalry + prov.troops.scouts;
    if (total === 0) return null;

    let size = 16;
    if (total > 100) size = 24;
    else if (total > 50) size = 20;

    return (
      <div className="flex flex-col items-center gap-1">
        <div className="flex gap-1 bg-black/20 p-1 rounded-full backdrop-blur-sm border border-white/10">
          {prov.troops.infantry > 0 && <Shield size={size / 2} className="text-slate-100 drop-shadow-md" />}
          {prov.troops.archers > 0 && <Swords size={size / 2} className="text-slate-200 drop-shadow-md" />}
          {prov.troops.cavalry > 0 && <Activity size={size / 2} className="text-amber-400 drop-shadow-md" />}
        </div>
        <div className="bg-[#1a0f0a] px-1.5 xs:px-2.5 py-0.5 xs:py-1 rounded-full text-[10px] xs:text-sm font-bold text-white leading-none border-1 xs:border-2 border-amber-500/50 shadow-lg">
          {total}
        </div>
      </div>
    );
  ...
  return (
    <div className="w-full h-full flex items-center justify-center pointer-events-none p-1 xs:p-2 md:p-4">
      <div
        className="relative parchment-bg rounded-lg xs:rounded-xl overflow-hidden shadow-2xl border-2 xs:border-4 md:border-8 border-[#2c1810] touch-none pointer-events-auto flex items-center justify-center"
          ...
          {(Object.values(gameState.provinces) as Province[]).map(prov => {
            ...
            return (
              ...
              >
                ...
                {isVisible && prov.terrain === 'mountains' && (
                ...
                {isVisible && (
                  <g className="pointer-events-none">
                    ...
                    <text
                      ...
                      y={prov.center[1] - 18}
                      textAnchor="middle"
                      className="text-[10px] xs:text-[12px] md:text-[14px] font-black fill-[#1a0f0a] uppercase tracking-[0.15em] medieval-title drop-shadow-[0_2px_1px_rgba(255,255,255,0.5)]"
                    ...
                    {owner && owner.capitalId === prov.id && (
                      <foreignObject
                        ...
                        height="12"
                      >
                        <Crown size={10} className="text-amber-600 drop-shadow-sm" />
                      </foreignObject>
                    )}
                  </g>
                )}
            ...
            {marchAnimations.map(anim => {
              ...
              return (
                ...
                >
                  ...
                  <text y="18" textAnchor="middle" className="text-[6px] xs:text-[8px] font-bold fill-white stroke-black stroke-[2px]" style={{ paintOrder: 'stroke' }}>
                  ...
                  <span className="text-[8px] font-bold uppercase text-slate-400 flex items-center gap-1">
                    ...
                    {gameState.provinces[hoveredProvinceId].strategicResource === 'iron' ? 'Ferro' :
                      gameState.provinces[hoveredProvinceId].strategicResource === 'wood' ? 'Madeira' :
                        gameState.provinces[hoveredProvinceId].strategicResource === 'horses' ? 'Cavalo' :
              ...
              <div className="space-y-2">
                <div className="grid grid-cols-3 gap-1 text-[9px] text-center">
                  <div className="bg-slate-800 p-1 rounded">
                    ...
                    <div className="font-bold">{gameState.provinces[hoveredProvinceId].troops?.infantry ?? 0}</div>
                  </div>
                  <div className="bg-slate-800 p-1 rounded">
                    <div className="text-slate-500">Arq</div>
                    <div className="font-bold">{gameState.provinces[hoveredProvinceId].troops?.archers ?? 0}</div>
                  </div>
                  <div className="bg-slate-800 p-1 rounded">
                    <div className="text-slate-500">Cav</div>
                    <div className="font-bold">{gameState.provinces[hoveredProvinceId].troops?.cavalry ?? 0}</div>
                  </div>
                </div>
