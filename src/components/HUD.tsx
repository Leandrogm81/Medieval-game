...
export const HUD: React.FC<HUDProps> = ({
  ...
  return (
    <>
      ...
      <button
        onClick={onToggleHud}
        className={`hud-toggle-btn fixed top-2 right-2 xs:top-4 xs:right-4 z-50 p-1.5 xs:p-2 sm:p-3 bg-slate-800/80 backdrop-blur-md border border-slate-700/50 rounded-full text-white shadow-xl transition-all active:scale-95 ${isHudOpen ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
        title={isHudOpen ? "Fechar Menu" : "Abrir Menu"}
      >
        <BarChart3 size={14} className="sm:w-5 sm:h-5" />
      </button>
                              ...
                              <div className="grid grid-cols-4 gap-1">
                                {(['infantry', 'archers', 'cavalry', 'scouts'] as UnitType[]).map(type => (
                                  <button
                                    ...
                                    className={`py-1 text-[8px] font-bold uppercase rounded border transition-all ${selectedUnitType === type
