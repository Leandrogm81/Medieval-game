...
import { 
  ...
  Settings, Maximize2, PlusCircle, HelpCircle, ChevronLeft, ChevronRight
} from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { ACTION_COSTS, BUILDING_PRODUCTION, BUILDING_STATS, UNIT_STATS } from '../logic/game-constants';
import { getTradeRate } from '../logic/economyLogic';
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      ...
                      <div className="w-full h-2 bg-stone-700 rounded-full overflow-hidden">
                        <div
                          className={`h-full rounded-full ${
                            selectedProvince.loyalty < 30 ? 'bg-red-500' : selectedProvince.loyalty > 70 ? 'bg-green-500' : 'bg-amber-500'
                          }`}
                          style={{ width: `${Math.max(0, Math.min(100, selectedProvince.loyalty))}%` }}
                        />
                      </div>
                    </div>
                    ...
                    <div className="bg-stone-900/40 border border-stone-700/40 rounded-sm p-2">
                      ...
                      <div className="w-full h-2 bg-stone-700 rounded-full overflow-hidden">
                        <div
                          className={`h-full rounded-full ${
                            provinceStability >= 80 ? 'bg-green-500' : provinceStability >= 50 ? 'bg-amber-500' : provinceStability >= 20 ? 'bg-orange-500' : 'bg-red-500'
                          }`}
                          style={{ width: `${Math.max(0, Math.min(100, provinceStability))}%` }}
                        />
                      </div>
                    </div>
