...
export const HUD: React.FC<HUDProps> = ({
  ...
  const totalTroops = playerProvinces.reduce((sum, p) => sum + (p.troops.infantry + p.troops.archers + p.troops.cavalry + p.troops.scouts), 0);
  ...
  const baseFoodIncome = playerProvinces.reduce((sum, p) => {
    ...
    if (p.strategicResource === 'horses') income += 5 * efficiency;
    return sum + income;
  }, 0);
  ...
  const getResourceIcon = (resource?: string) => {
    switch (resource) {
      ...
      case 'horses': return <Activity size={14} className="text-amber-600" />;
  ...
  return (
    <>
      ...
      <button
        onClick={onToggleHud}
        className={`hud-toggle-btn absolute top-4 right-4 z-50 p-3 bg-slate-800/80 backdrop-blur-md border border-slate-700/50 rounded-full text-white shadow-xl transition-all active:scale-95 ${isHudOpen ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
      ...
      <div
        className={`hud-docked bg-slate-900/95 backdrop-blur-xl border-l border-white/10 flex flex-col shadow-2xl z-40 transition-all duration-300 ease-in-out absolute right-0 top-0 h-full w-[200px] xs:w-[220px] sm:w-[260px] md:w-[300px] lg:w-[360px] ${isHudOpen ? 'translate-x-0 opacity-100' : 'translate-x-[120%] opacity-0'
          }`}
      >
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden custom-scrollbar">
          <div className="p-1.5 xs:p-2 md:p-3 space-y-1.5 xs:space-y-2">
            {/* Header with Fullscreen option */}
            <div className="flex justify-between items-center mb-0.5 xs:mb-1 gap-1">
              <h1 className="text-[10px] xs:text-xs md:text-sm font-serif font-bold text-slate-100 flex items-center gap-0.5 xs:gap-1 min-w-0">
                <Crown className="text-amber-500 shrink-0" size={10} /> <span className="truncate">{playerRealm.name}</span>
              </h1>
            ...
            {/* Resources Grid */}
            <div className="grid grid-cols-2 gap-0.5 xs:gap-1 mb-1 xs:mb-1.5">
              <div className="bg-slate-900/40 p-0.5 xs:p-1 rounded border border-slate-700/30 flex flex-col justify-between h-7 xs:h-8 shadow-inner">
                <div className="flex justify-between items-center text-[6px] xs:text-[7px] md:text-[8px] text-slate-500 uppercase font-black tracking-tighter">
                ...
                <div className="text-[10px] xs:text-xs md:text-sm font-black text-amber-500 font-serif leading-none tracking-tight">{Math.floor(playerRealm.gold)}</div>
              </div>
          ...
          <div className="px-3 py-1.5 bg-slate-800/50 border-b border-slate-700/50 shrink-0">
            <div className="flex bg-slate-900 rounded-lg p-0.5 border border-slate-700 shadow-inner">
              <button onClick={() => setActiveTab('province')} className={`flex-1 flex flex-col sm:flex-row items-center justify-center gap-0.5 xs:gap-1 py-1 xs:py-1.5 sm:py-2 rounded text-[8px] xs:text-[9px] sm:text-[10px] md:text-xs font-black uppercase transition-all ${activeTab === 'province' ? 'bg-amber-600 text-white shadow-md' : 'text-slate-500 hover:text-slate-300'}`}>
                <MapIcon size={10} className="mb-0.5 sm:mb-0" /> <span className="hidden xs:inline">Reinos</span>
              </button>
              <button onClick={() => setActiveTab('market')} className={`flex-1 flex flex-col sm:flex-row items-center justify-center gap-0.5 xs:gap-1 py-1 xs:py-1.5 sm:py-2 rounded text-[8px] xs:text-[9px] sm:text-[10px] md:text-xs font-black uppercase transition-all ${activeTab === 'market' ? 'bg-amber-600 text-white shadow-md' : 'text-slate-500 hover:text-slate-300'}`}>
                <ShoppingCart size={10} className="mb-0.5 sm:mb-0" /> <span className="hidden xs:inline">Mercado</span>
              </button>
              <button onClick={() => setActiveTab('diplomacy')} className={`flex-1 flex flex-col sm:flex-row items-center justify-center gap-0.5 xs:gap-1 py-1 xs:py-1.5 sm:py-2 rounded text-[8px] xs:text-[9px] sm:text-[10px] md:text-xs font-black uppercase transition-all ${activeTab === 'diplomacy' ? 'bg-amber-600 text-white shadow-md' : 'text-slate-500 hover:text-slate-300'}`}>
                <Handshake size={10} className="mb-0.5 sm:mb-0" /> <span className="hidden xs:inline">Política</span>
              </button>
            </div>
          </div>

          {/* Map View Modes Overlay (Compact) */}
          <div className="px-1.5 xs:px-2 sm:px-3 py-1 xs:py-1.5 bg-slate-900 border-b border-slate-800 shrink-0 flex flex-wrap gap-0.5 xs:gap-1 items-center justify-center">
            <span className="text-[8px] xs:text-[9px] font-black text-slate-700 uppercase tracking-tighter mr-0.5 xs:mr-1 w-full text-center sm:w-auto sm:text-left mb-0.5 sm:mb-0">Visão</span>
            {[
              ...
              <button
                ...
                className={`flex items-center gap-0.5 xs:gap-1 px-1 sm:px-2 py-0.5 xs:py-1 sm:py-1.5 rounded border transition-all flex-1 justify-center sm:flex-none ${viewMode === mode.id ? `bg-${mode.color}-600/20 border-${mode.color}-500 text-${mode.color}-400 shadow-sm` : 'bg-slate-800 border-transparent text-slate-500 hover:bg-slate-700'}`}
              >
                {mode.icon} <span className="text-[8px] xs:text-[9px] sm:text-[10px] font-black uppercase">{mode.label}</span>
              </button>
            ))}
          </div>
          ...
          {gameState.currentEvent && (
            <div className={`p-2 border-b shrink-0 ${gameState.currentEvent.type === 'positive' ? 'bg-green-900/30 border-green-500/50' :
              ...
              <div className="flex items-center gap-1.5 mb-0.5">
                ...
                <span className={`text-[9px] font-black uppercase tracking-widest ${gameState.currentEvent.type === 'positive' ? 'text-green-400' :
          ...
          <div className="flex-1 p-3 overflow-y-auto">
            {activeTab === 'market' ? (
              <div className="space-y-4">
                <h2 className="text-base xs:text-lg font-serif font-bold text-slate-100 border-b border-slate-700 pb-1.5 flex items-center gap-1.5">
                  <ShoppingCart size={14} className="text-amber-500" /> Mercado Global
                </h2>

                <div className="space-y-2.5">
                  <div className="bg-slate-800 p-2.5 rounded-lg border border-slate-700">
                    <div className="flex justify-between items-center mb-2">
                      <span className="flex items-center gap-1.5 text-xs xs:text-sm font-bold text-green-400"><Wheat size={12} /> Comida</span>
                      <span className="text-[8px] xs:text-[10px] text-slate-500">Estoque: {playerRealm.food || 0}</span>
                    </div>
                    <div className="grid grid-cols-2 gap-1.5">
                      <button onClick={() => onAction('buy_food')} className="flex items-center justify-between px-2 py-2 bg-slate-800 hover:bg-slate-700 rounded transition-colors group">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-[8px] xs:text-[10px] font-bold">Comprar 50</span>
                        </div>
                        <span className="text-[8px] xs:text-[10px] text-yellow-500 font-black tracking-tighter bg-yellow-500/10 px-1 rounded">30O</span>
                      </button>
                      <button onClick={() => onAction('sell_food')} className="flex items-center justify-between px-2 py-2 bg-slate-800 hover:bg-slate-700 rounded transition-colors group">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-[8px] xs:text-[10px] font-bold">Vender 50</span>
                        </div>
                        <span className="text-[8px] xs:text-[10px] text-yellow-500 font-black tracking-tighter bg-yellow-500/10 px-1 rounded">21O</span>
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-800 p-2.5 rounded-lg border border-slate-700">
                    <div className="flex justify-between items-center mb-2">
                      <span className="flex items-center gap-1.5 text-xs xs:text-sm font-bold text-slate-300"><Hammer size={12} /> Materiais</span>
                      <span className="text-[8px] xs:text-[10px] text-slate-500">Estoque: {playerRealm.materials || 0}</span>
                    </div>
                    <div className="grid grid-cols-2 gap-1.5">
                      <button onClick={() => onAction('buy_materials')} className="flex items-center justify-between px-2 py-2 bg-slate-800 hover:bg-slate-700 rounded transition-colors group">
                        ...
                        <span className="text-[8px] xs:text-[10px] text-yellow-500 font-black tracking-tighter bg-yellow-500/10 px-1 rounded">50O</span>
                      </button>
                      <button onClick={() => onAction('sell_materials')} className="flex items-center justify-between px-2 py-2 bg-slate-800 hover:bg-slate-700 rounded transition-colors group">
                        ...
                        <span className="text-[8px] xs:text-[10px] text-yellow-500 font-black tracking-tighter bg-yellow-500/10 px-1 rounded">35O</span>
                      </button>
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-4 h-full flex flex-col">
                <h2 className="text-base xs:text-lg font-serif font-bold text-slate-100 border-b border-slate-700 pb-1.5 flex items-center gap-1.5 shrink-0">
                  <Handshake size={14} className="text-purple-500" /> Diplomacia
                </h2>
                ...
                <div className="space-y-1 text-[10px] md:text-xs">
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-slate-400 flex items-center gap-1 text-[10px] capitalize">
                      ...
                      {selectedProv.terrain === 'mountains' && <Mountain size={10} />}
                      ...
                      {selectedProv.ownerId === playerRealm.id ? (
                          ...
                          {isRecruiting ? (
                            <div className="bg-slate-800 p-1.5 rounded-lg border border-blue-500/50 space-y-1.5">
                              ...
                              <div className="grid grid-cols-4 gap-1.5 mb-1.5">
                                {(['infantry', 'archers', 'cavalry', 'scouts'] as UnitType[]).map(type => (
                                  <button
                                    ...
                                    className={`py-2 text-[10px] font-bold uppercase rounded border transition-all ${selectedUnitType === type
                              ...
                              <div className="space-y-1.5">
                                ...
                                <div className="flex justify-between items-end">
                                  <div className="flex flex-col gap-1">
                                    <span className="text-[9px] text-slate-400">Quantidade</span>
                                    <div className="flex gap-1.5 items-center">
                                      <input
                                        ...
                                        className="w-14 bg-slate-700 border border-slate-600 rounded px-2 py-1 text-xs font-bold text-white focus:outline-none focus:border-blue-500"
                                      />
                                      <div className="flex gap-1">
                                        {[0.25, 0.5, 1].map(ratio => {
                                          ...
                                          return (
                                            <button
                                              ...
                                              className="px-2 py-1 bg-slate-600 hover:bg-slate-500 rounded text-[9px] font-bold text-white"
                                            >
                                              {ratio === 1 ? 'M' : ratio === 0.5 ? '½' : '¼'}
                                            </button>
                                          );
                              ...
                              <button
                                ...
                                className={`w-full py-2.5 mt-2 rounded text-xs font-bold transition-colors ${(() => {
                                  const stats = UNIT_STATS[selectedUnitType] as any;
                                  if (!stats.requires) return false;
                                  return !(Object.values(gameState.provinces) as Province[]).some(p => p.ownerId === playerRealm.id && p.strategicResource === stats.requires);
                                })()
                            ...
                            <button
                              ...
                              className="w-full flex items-center justify-center gap-1.5 py-2.5 bg-blue-600 hover:bg-blue-500 rounded font-bold transition-colors text-xs"
                            >
                              <PlusCircle size={14} /> Recrutar Unidades
                            </button>
                          )}

                          <div className="pt-2">
                            <span className="text-[9px] font-bold text-slate-500 uppercase tracking-wider">Construção ({ACTION_COSTS.build} AP)</span>
                            <div className="grid grid-cols-1 gap-1.5 mt-1.5">
                              <button onClick={() => onAction('build_farms')} className="flex items-center justify-between px-2 py-2 bg-slate-800 hover:bg-slate-700 rounded text-xs transition-colors border border-slate-700">
                                <span className="flex items-center gap-1.5"><Wheat size={12} className="text-green-500" /> Fazenda</span>
                                <span className="text-[9px] text-slate-400 font-bold">{BUILDING_STATS.farms.gold}O, {BUILDING_STATS.farms.materials}M</span>
                              </button>
                              <button onClick={() => onAction('build_mines')} className="flex items-center justify-between px-2 py-2 bg-slate-800 hover:bg-slate-700 rounded text-xs transition-colors border border-slate-700">
                                <span className="flex items-center gap-1.5"><Pickaxe size={12} className="text-yellow-500" /> Mina</span>
                                <span className="text-[9px] text-slate-400 font-bold">{BUILDING_STATS.mines.gold}O, {BUILDING_STATS.mines.materials}M</span>
                              </button>
                              <button onClick={() => onAction('build_workshops')} className="flex items-center justify-between px-2 py-2 bg-slate-800 hover:bg-slate-700 rounded text-xs transition-colors border border-slate-700">
                                <span className="flex items-center gap-1.5"><Factory size={12} className="text-slate-400" /> Oficina</span>
                                <span className="text-[9px] text-slate-400 font-bold">{BUILDING_STATS.workshops.gold}O, {BUILDING_STATS.workshops.materials}M</span>
                              </button>
                              <button onClick={() => onAction('build_courts')} className="flex items-center justify-between px-2 py-2 bg-slate-800 hover:bg-slate-700 rounded text-xs transition-colors border border-slate-700">
                                <span className="flex items-center gap-1.5"><Home size={12} className="text-blue-400" /> Tribunal</span>
                                <span className="text-[9px] text-slate-400 font-bold">{BUILDING_STATS.courts.gold}O, {BUILDING_STATS.courts.materials}M</span>
                              </button>
                            </div>
                          </div>
                          ...
                          <div className="pt-1.5">
                            ...
                            <div className="grid grid-cols-2 gap-1.5 mt-1.5">
                              <button
                                ...
                                className="flex items-center justify-center gap-1.5 py-2 bg-slate-700 hover:bg-slate-600 rounded font-bold transition-colors text-xs"
                              >
                                <ArrowRight size={12} /> Mover
                              </button>
                              <button
                                onClick={onRoute}
                                className="flex items-center justify-center gap-1.5 py-2 bg-indigo-700 hover:bg-indigo-600 rounded font-bold transition-colors text-xs"
                              >
                                <Route size={12} /> Rota
                              </button>
                            </div>
                            ...
                            {(selectedProv.army.scouts || 0) > 0 && (
                              <button
                                ...
                                className="w-full mt-1.5 flex items-center justify-center gap-1.5 py-2 bg-emerald-800 hover:bg-emerald-700 rounded font-bold transition-colors text-xs"
                              >
                                <Eye size={12} /> Despachar Batedores
                              </button>
                            )}
                            <div className="grid grid-cols-1 mt-2">
                              <button onClick={() => onMapAction('attack')} className="flex items-center justify-center gap-1.5 py-2.5 bg-red-700 hover:bg-red-600 rounded font-black transition-colors text-xs">
                        ...
                        <>
                          {playerRealm.wars.includes(selectedProv.ownerId) ? (
                            <>
                              <div className="p-1.5 mb-1.5 bg-red-900/40 border border-red-500/50 rounded text-center">
                                <span className="text-red-400 font-bold text-xs uppercase tracking-wider">Em Guerra</span>
                              </div>
                              <button
                                onClick={() => onAction('offer_peace')}
                                className="w-full flex items-center justify-center gap-1.5 py-2.5 bg-purple-600 hover:bg-purple-500 rounded font-bold transition-colors text-xs mb-2"
                              >
                                <Handshake size={14} /> Oferecer Paz ({ACTION_COSTS.diplomacy} AP)
                              </button>
                              <p className="text-[10px] text-slate-400 text-center px-1 mt-1">
                            ...
                            <>
                              <button
                                ...
                                className="w-full flex items-center justify-center gap-1.5 py-2.5 bg-red-600 hover:bg-red-500 rounded font-bold transition-colors text-xs mb-2 text-white"
                              >
                                <Swords size={14} /> Declarar Guerra ({ACTION_COSTS.diplomacy} AP)
                              </button>

                              <div className="grid grid-cols-2 gap-1.5 mt-2">
                                <button
                                  onClick={() => onAction('send_gift')}
                                  className="flex items-center justify-center gap-1.5 py-2 bg-purple-600 hover:bg-purple-500 rounded font-bold transition-colors text-xs"
                                >
                                ...
                                <button
                                  ...
                                  className={`flex items-center justify-center gap-1.5 py-2 rounded font-bold transition-colors text-xs ${playerRealm.pacts.includes(selectedProv.ownerId) ? 'bg-orange-700 hover:bg-orange-600' : 'bg-blue-600 hover:bg-blue-500'}`}
                                >
                                  <Shield size={14} /> {playerRealm.pacts.includes(selectedProv.ownerId) ? 'Quebrar Pacto' : 'Pacto'}
                                </button>
                                <button
                                  onClick={() => onAction('propose_alliance')}
                                  className="flex items-center justify-center gap-1.5 py-2 bg-indigo-600 hover:bg-indigo-500 rounded font-bold transition-colors text-xs"
                                >
                                  <Zap size={14} /> Aliança
                                </button>
                                <button
                                  onClick={() => onAction('trade_route')}
                                  className="flex items-center justify-center gap-1.5 py-2 bg-emerald-600 hover:bg-emerald-500 rounded font-bold transition-colors text-xs"
                                >
                              ...
                              <div className="grid grid-cols-2 gap-1.5 mt-1.5">
                                <button
                                  ...
                                  className="flex items-center justify-center gap-1.5 py-2 bg-amber-700 hover:bg-amber-600 rounded font-bold transition-colors text-xs"
                                >
